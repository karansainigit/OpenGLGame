...
#pragma comment(lib, "opengl32.lib")        // search for libraries
...
BOOL CreateGLWindow(char* title, int width, int height, int bits, bool fullscreenflag)
{
	...
	Scene->reSizeScene(WindowRect.right-WindowRect.left, WindowRect.bottom-WindowRect.top);
							...
							LPARAM	lParam)			// Additional Message Information
{
	...
	switch (uMsg)									// Check For Windows Messages
	{
		...
		{
			keys[wParam] = TRUE;
			Scene->winMsg(hWnd, uMsg, wParam, lParam);					// If So, Mark It As TRUE
			return 0;								// Jump Back
		}
					...
					int			nCmdShow)			// Window Show State
{
	...
	if (!CreateGLWindow("Game",fullscreenWidth,fullscreenHeight,256,fullscreen))
	{
	...
	while(!done)									// Loop That Runs While done=FALSE
	{
		...
		else										// If There Are No Messages
		{
			...
			if (!active  || keys[VK_END])	// Active?  Was There A Quit Received?
			{
